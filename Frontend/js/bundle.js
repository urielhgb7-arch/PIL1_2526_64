...
async function fetchAPI(endpoint, method = "GET", body = null) {
  ...
  try {
    ...
    if (response.status === 401) {
      const pub =
        /^\/(pages\/)?($|index\.html|signin\.html|signup\.html|reset-password\.html)/.test(
...
const API = {
  auth: {
    ...
    changePassword: (data) => {
      const payload = {
        old_password:
          data.old_password || data.ancien_mot_de_passe || data.oldPassword,
        new_password:
          data.new_password || data.nouveau_mot_de_passe || data.newPassword,
      };
      return fetchAPI("/auth/change-password", "PUT", payload);
    },
    deleteAccount: (password) =>
      fetchAPI("/auth/delete-account", "DELETE", { password }),
  },
  ...
  profile: {
    ...
    removeCompetence: (data) =>
      fetchAPI("/profile/competences", "DELETE", data),
    ...
    addDisponibilite: (data) =>
      fetchAPI("/profile/disponibilites", "POST", data),
    removeDisponibilite: (data) =>
      fetchAPI("/profile/disponibilites", "DELETE", data),
    activateCompetence: (matiereId) =>
      fetchAPI(`/profile/competences/${matiereId}/activate`, "PUT"),
    deactivateCompetence: (matiereId) =>
      fetchAPI(`/profile/competences/${matiereId}/deactivate`, "PUT"),
  },
  ...
  matching: {
    getSuggestions: (filters = {}) => {
      ...
      return fetchAPI(
        `/matches/suggestions${params ? "?" + params : ""}`,
        "GET",
      );
    },
    requestMatch: (studentId, body) =>
      fetchAPI(`/matches/${studentId}/request`, "POST", body),
  ...
  messages: {
    ...
    getHistory: (convId) =>
      fetchAPI(`/conversations/${convId}/messages`, "GET"),
    send: (convId, text) =>
      fetchAPI(`/conversations/${convId}/messages`, "POST", { contenu: text }),
  },

  notifications: {
    getAll: (unreadOnly = false) =>
      fetchAPI(`/notifications${unreadOnly ? "?unread_only=true" : ""}`, "GET"),
...
const DEFAULT_MATIERES = [
  ...
  {
    id: 24,
    nom: "Administration de bases de données",
    filiere: "RSI",
    annee: "M1",
  },
...
function getMatiersByFiliere(filiere = null) {
  ...
  return _matieresCache.filter((m) => {
    ...
    return (
      mFiliere.toLowerCase().includes(filiere.toLowerCase()) ||
...
async function fetchUnreadCount() {
  try {
    ...
    const unread =
      d.unread_count ??
      (d.notifications || []).filter((n) => !n.is_read && !n.lu).length;
    return unread;
  } catch (e) {
